...
pumpkins %>% ggplot(aes(x = day, y = price)) +
  ...
  labs(x = "Day of the Year", 
       y = "Pumpkin Price", 
       title = "Average Pumpkin Sales Throughout the Year") +
  theme_minimal()
...
```{r}
## New data frame with only bushel packages
...
new_pumpkins %>% 
  ...
  geom_point(size = 1.6) +
  labs(title = "Price per bushel of pumpkins throughout the year",
       x = "Day of Year",
       y = "Price ($)")
```
...
**Bushels of pumpkins are only sold in about the last 100 days of the year, so we are missing information at the beginning of the year. Also, there are many different prices for the same day**

How do we make it more useful? To get charts to display useful data, you usually need to group the data somehow.

**It would be more useful to group by day and get an average for each day**
...
**Using `unique(baked_pumpkins$city_name)` there are 10 cities represented in the data set**
...
**Package is most highly correlated with price. This could be because the bushels are based on weight, so more weight = higher price**
...
**The package type is a categorical variable and therefore not a continuous variable which is not very compatible with a linear regression fit. Also, this line is underfitting the data and is therefore missing the true data signal.**
